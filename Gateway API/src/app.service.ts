import { BadRequestException, Get, HttpStatus, Injectable, Param, Post, Query, Res } from '@nestjs/common';
...
export class AppService {
  ...
  async getReservations(userName: string): Promise<any> {
    ...
    var res = <Array<any>>(await axios.get(global.HOSTS.microService3+`/api/v1/reservation?username=${userName}`)).data;
  ...
  async postReservations(userName: string, data: TakeBookRequestDto): Promise<any> {
    ...
    let userRecords:Array<any> = (await axios.get(global.HOSTS.microService3+`/api/v1/reservation?username=${userName}`)).data as Array<any>;
  ...
  async getUserRating(username: string): Promise<any> {
    console.log((await axios.get(global.HOSTS.microService4+`/api/v1/rating?username=${username}`)).data);
